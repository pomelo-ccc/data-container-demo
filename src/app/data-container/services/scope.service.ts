...
export class ScopeService {
    /** 父级 Scope 引用 ($parent) */
    private _parent: ScopeService | null = null;
    ...
    readonly data: Signal<Record<string, any>> = computed(() => {
        ...
        if (parentData) {
            // 使用原型链继承: localData 作为自身属性，parentData 作为原型
            const inherited = Object.create(parentData);
            Object.assign(inherited, localData);
            return inherited;
        }
    ...
    private _updateNamedScopes(): void {
        ...
        const newNamed: NamedScopes = { ...parentNamed };

        if (this._id) {
            newNamed[this._id] = this;
        }

        this._namedScopes.set(newNamed);
    ...
    getSiblings(): ScopeService[] {
        ...
        return this._parent.getChildren().filter(child => child._id !== this._id);
    }
    ...
    setValue(key: string, value: any, bubble: boolean = false): void {
        if (bubble && this._parent) {
            this._parent.setValue(key, value, bubble);
        } else {
    ...
    evaluateExpression(expression: string): any {
        ...
        // 1. 处理 ${expression} 形式的完整表达式
        // 例如: "${tableVisible !== false}" -> "tableVisible !== false"
        if (expression.startsWith('${') && expression.endsWith('}')) {
            const code = expression.slice(2, -1);
            try {
                // 创建一个代理对象，对未定义的属性返回 undefined 而不是抛出错误
                const safeData = new Proxy(data, {
                    has: () => true, // with 语句会检查属性是否存在
                    get: (target, prop) => {
